...
from models.minkloc import MinkLoc
from models.minkloconly import MinkLocOnly
...
from models_adafusionv2.model_factory_adafusionv2 import AdaFusionV2
from models_adafusionv2.model_factory_adafusionv2 import model_factory_adafusionv2
...
def make_params_l(model):
    ...
    if isinstance(model, DistilImageFE):
    ...
    elif isinstance(model, MinkLoc):
        params_l.append({'params': model.parameters(), 'lr': args.cloud_lr}) # 1e-3
        ...
        params_l.append({'params': model.parameters(), 'lr': args.cloud_lr})


    elif isinstance(model, MinkLocOnly):
        params_l.append({'params': model.parameters(), 'lr': args.cloud_lr})
    ...
    elif isinstance(model, AdaFusionV2):
        params_l.append({'params': model.image_fe.parameters(), 'lr': args.image_lr})
        params_l.append({'params': model.cloud_fe.parameters(), 'lr': args.cloud_lr})
...
def do_train(dataloaders, params: MinkLocParams):
    ...
    elif args.model == 'minkloconly':
        planes = [int(e) for e in args.teacher_minkloconly_planes.split('_')]
        layers = [int(e) for e in args.teacher_minkloconly_layers.split('_')]
        model = MinkLoc(
            in_channels=1, feature_size=args.teacher_minkloconly_dim, output_dim=args.teacher_minkloconly_dim,
            planes=planes, layers=layers, num_top_down=args.teacher_minkloconly_topdown,
    ...
    elif args.model == 'adafusionv2':
        planes = [int(e) for e in args.teacher_adav2_cloud_planes.split('_')]
        layers = [int(e) for e in args.teacher_adav2_cloud_layers.split('_')]
        model = model_factory_adafusionv2(
                            cloud_fe_size=args.teacher_adav2_cloud_fe_dim,
                            image_fe_size=args.teacher_adav2_image_fe_dim,
                            cloud_planes=planes,
                            cloud_layers=layers,
                            cloud_topdown=args.teacher_adav2_cloud_topdown,
                            image_useallstages=args.teacher_adav2_image_useallstages,
                            image_fe=args.teacher_adav2_image_fe,
                            useattenres=args.teacher_adav2_useattenres
        )
    ...
    elif args.student_model == 'minkloconly':
        planes = [int(e) for e in args.student_minkloconly_planes.split('_')]
        layers = [int(e) for e in args.student_minkloconly_layers.split('_')]
        model_stu = MinkLocOnly(
                            in_channels=1, 
                            feature_size=args.student_output_dim, 
                            output_dim=args.student_output_dim,
                            planes=planes, 
                            layers=layers, 
                            num_top_down=args.student_minkloconly_topdown,
                            conv0_kernel_size=5, 
                            block='ECABasicBlock', 
                            pooling_method='GeM'
        )
    ...
    for epoch in range(args.epochs):
        ...
        for i_batch, (batch_dict, positives_mask, negatives_mask, filenames) in tqdm(enumerate(dataloaders['train'])):
            ...
            if isinstance(model, DistilImageFE):
                ...
                now_image_lr = 0
                now_cloud_lr = optimizer.param_groups[0]["lr"]
                now_image_lr_stu = 0
                now_cloud_lr_stu = optimizer_stu.param_groups[0]["lr"]
            elif isinstance(model, MinkLocMultimodal):
                now_image_lr = optimizer.param_groups[0]["lr"]
