...
class MinkFPNAdaFusion(ResNetBase):
    ...
    def network_initialization(self, in_channels, out_channels, D):
        ...
        self.avg1x1 = ME.MinkowskiGlobalAvgPooling()
    ...
    def forward(self, x):
        ...
        for ndx, (conv, bn, block) in enumerate(zip(self.convs, self.bns, self.blocks)):
            ...
            x = self.relu(x)
            x = block(x)
            _x_avg = self.avg1x1(x)
            _x_avg = _x_avg.F
            output_dict[f'layer{ndx+1}'] = x
            output_dict[f'layer{ndx+1}_avg'] = _x_avg
...
class MinkLocAdaFusion(torch.nn.Module):
    def __init__(self, in_channels, feature_size, output_dim, planes, layers, num_top_down, conv0_kernel_size,
        ...
        self.backbone = MinkFPNAdaFusion(in_channels=in_channels, out_channels=self.feature_size, num_top_down=num_top_down,
    ...
    def forward(self, batch):
        ...
        x_gem = self.pooling(x_feat)
        


        cloud_layer1_avg = x_dict['layer1_avg']
        cloud_layer2_avg = x_dict['layer2_avg']
        cloud_layer3_avg = x_dict['layer3_avg']

        cloud_atten = torch.cat([
            cloud_layer1_avg, 
            cloud_layer2_avg, 
            cloud_layer3_avg
            ], dim=1)
        cloud_atten = self.attn(cloud_atten)
        cloud_atten = cloud_atten * x_gem
        if self.useattenres:
            cloud_atten += x_gem 
        ...
        output_dict = {
            ...
            'embedding':x_gem,
            'cloud_atten':cloud_atten
            }

        # return {
        #     'output_cloud_feat': x_feat,
        #     'output_cloud_gem': x_gem,
        #     'output_cloud_layer1_avg': x_dict['layer1_avg'],
        #     'output_cloud_layer2_avg': x_dict['layer2_avg'],
        #     'output_cloud_layer3_avg': x_dict['layer3_avg'],
        #     'embedding':x_gem
        #     }
        return x_gem, output_dict
